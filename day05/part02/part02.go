...
func Filter(priorities map[int][]int, instructions [][]int) [][]int {
	invalid := make([][]int, 0, len(instructions))

	for i := range instructions {
		if !checkInstruction(instructions[i], priorities) {
			invalid = append(invalid, instructions[i])
		}
	}
...
func ReorderInvalid(priorities map[int][]int, instructions [][]int) [][]int {
	valid := make([][]int, 0, len(instructions))

	for i := range instructions {
		if checkAndReorderInstruction(instructions[i], priorities) {
...
func checkAndReorderInstruction(ns []int, priorities map[int][]int) bool {
	for idx := 0; idx < len(ns); idx++ {
		after := priorities[ns[idx]]

		if idx > 0 && containsPrev(ns[:idx], after) {
			swapped := make([]int, len(ns))
			copy(swapped, ns)
			var ok bool

			for inner := idx; inner < len(ns); inner++ {
				swapped, ok = swapPrev(ns, inner)
				if ok && checkInstruction(swapped, priorities) {
					return true
				}
			}

			return checkAndReorderInstruction(swapped, priorities)
		}
	}
...
func swapPrev(ns []int, idx int) ([]int, bool) {
	if idx <= 0 || idx >= len(ns) {
		return ns, false
	}
